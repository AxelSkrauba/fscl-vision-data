...
def main(config_path: str, max_per_taxon: int = None):
    ...
    if place_id:
        logger.info("Using Geo Filter: Place ID")
    ...
    for taxon_filter in taxa:
        ...
        logger.info(
            f"Fetching observations for: {taxon_name} (taxon_id={taxon_id}, max={max_obs})")
        ...
        try:
            ...
            if obs:
                ...
                if str(first_id) != str(taxon_id):
                    logger.debug(
                        f"   (Taxon mismatch note: Requested {taxon_id}, got {first_id} - usually subspecies)")
    ...
    logger.info("=" * 50)
    logger.info(
        f"Total: {len(all_observations)} observations, {len(by_species)} species")
